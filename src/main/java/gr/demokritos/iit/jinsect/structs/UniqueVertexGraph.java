...
public class UniqueVertexGraph extends WeightedGraphImpl implements Serializable {
	...
	public HashMap<Edge, Double> UniqueEdges;
    ...
    public UniqueVertexGraph() {
		...
		UniqueEdges = new HashMap<Edge, Double>();
...
//            }
		...
		if (e != null) {
			...
			/* get weight of edge (unitary if the edge is unweighted) */
			if (e instanceof WeightedEdge) 
				weight = ((WeightedEdge) e).getWeight();
			else 
				weight = 1.0;

			/* add edge to the hash map */
			UniqueEdges.put(e, weight);
		}
    ...
    public synchronized WeightedEdge addEdge(Vertex vHead, Vertex vTail, double dWeight)
		...
		/* add to edges map if not null */
		if (e != null) {
			UniqueEdges.put(e, e.getWeight());
		}
